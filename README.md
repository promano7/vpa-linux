...
| 2 — Capa gráfica (`ptcgraph`) | 🟡 En curso | Swap aplicado; SVGA fuera; `vpa.cfg`. **`SCREEN`, `TCOMBAT`, `MESSAGES` portados y compilan.** (`MESSAGES`: `Mem[Seg0040]` shift→`KbdFlags`, `EncodeObject` asm→Pascal.) Geometría/atributos de texto: a afinar viendo el binario. Frente actual: asm de `RST_TRN.PAS` (ficheros .rst/.trn). |
