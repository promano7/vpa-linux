...
| — Hallazgo VPACC/HULLFUNC | ⚠️ Bloqueo arquitectónico | El núcleo usa la API de hull-functions (`THullFuncQueryResult`, `EnumHullfuncs`) que vive en `CC/HULLFUNC.PAS`, dependiente de units **ausentes** (PCC2). **Alcance de la capa de compat (en `VPADATA`, ya identificado al llegar a `VPA4`):** (a) vars de estado del núcleo movidas a la rama VPACC-on (`BL0`, `bOver`, `mt0/mt1`, `MineN`, `IonN`, `WormN`, `UfoN`, `MarkN`, `AllH`, `ScanH`, `lsxy`, `dlship`, `iBeam/jBeam/jAmmo`, `GSOP`, `DScolor`…); (b) ~40 constantes `SPC_*` (de `HULLFUNC`); (c) tipo `THullFuncQueryResult` + stubs vacíos de `Enum*`. Plan: proveerlas siempre (VPACC off) y reimplementar `HULLFUNC` con **PCC2 como referencia** (ver §1). `iColo/iDefense/iRace` ya añadidas. |
