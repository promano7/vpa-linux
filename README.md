...
| — Capa compat VPACC/HULLFUNC | ✅ Desbloqueado | Añadida a `VPADATA` (rama `{$IFNDEF VPACC}`): vars de estado del núcleo (`BL0`, `bOver`, `mt0/mt1`, `MineN`…), consts (`iBeam/jBeam/jAmmo/iColo/iDefense/iRace`), ~40 constantes `SPC_*`, tipo `THullFuncQueryResult`. Los usos de `Enum*` del núcleo van bajo `{$IFDEF VPACC}`, así que **no** hicieron falta stubs. `IsHullFunc` (asm) portado a Pascal. **`VPA4` ya compila.** Reimplementar `HULLFUNC` real (PCC2) queda pendiente para restaurar la pantalla de habilidades. |
