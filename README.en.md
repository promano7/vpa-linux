...
| `ptccrt.pp` | **Modified** (VPA patch) | Base for self-contained recompilation of `ptcgraph`, and also carries several VPA patches in keyboard handling: exposes `PTCLastKbdFlags` (Shift/Ctrl/Alt of the last event, used as a fallback), treats the window's **[X]** button as **Alt-X** (quit saving) and sets `PTCQuitNoSave` for **Ctrl-Alt-X** (quit without saving), and emits **Alt+arrow** (`$9B/$9D/$98/$A0`) also in `kmTP7` mode —VPA's— not just in `kmGO32/kmFPWINCRT` (restores DOS TP7 behaviour). It also translates **Ctrl-+ / Ctrl--** (jump to last/first turn) into the DOS **grey** `+`/`-` scancodes (`$9000`/`$8E00`) by looking at the **character** instead of the key code, so they work the same from the numeric keypad and from the main row, on any layout (on `es`/`de`/`fr` the `+` is a key of its own and its `XK_plus` keysym is missing from `ptc`'s X11 table). With its modification notice. |
...
- [x] Working branch. ✅ Development happens on **`main`**. Every time a release ships, the state of `main` is **frozen** as a **`3.67.x`** branch (e.g. `3.67.2`), with a `v3.67.x` tag pointing at the exact commit; `main` always continues as the development branch for future fixes and improvements.
